...
static void disas_arm_insn(CPUState * env, DisasContext *s)
{
    ...
    if(start_tracing==1)fprintf(insn_trace_file_A,"%x\n",insn);
 //sue7785 2013.1.8 Add for timestamp lookup-table
#ifdef CONFIG_TRACE_TIME
        if(start_tracing) {
...
#ifdef TRANSFORMER_KIND
  	//printf("translate insn = %X\n");
	/*if(flag4M_on)*/if(start_tracing||flag4M_on)seg_cycle_cnt+=read_table(insn); //yhchiang 20160328
	//if(semihosting_enabled)local_tb_cycle += read_table(insn);
#endif
    }
}
...
static int disas_thumb2_insn(CPUState *env, DisasContext *s, uint16_t insn_hw1)
{
    ...
    if (!(arm_feature(env, ARM_FEATURE_THUMB2)
        ...
        insn = insn_hw1;
	//printf("%X ",insn);
	local_tb_cycle += read_thumb_table(insn);
        if ((insn & (1 << 12)) == 0) {
            ARCH(5);
            /* Second half of blx.  */
            offset = ((insn & 0x7ff) << 1);
                                                  ...
                                                  int search_pc)
{
    ...
    if (unlikely(env->singlestep_enabled)) {
        ...
        if (dc->condjmp) {
            gen_set_label(dc->condlabel);
            gen_set_condexec(dc);
            gen_goto_tb(dc, 1, dc->pc);//yhchiang 2016
	    //printf("conditional jump\n");
	    //tb_total_cycle_cnt += 30;
	    local_tb_cycle += 15;
	    pre_taken = 1;
            dc->condjmp = 0;
        }
