...
struct x86_uop_t
{
	...
	long long first_issue_try; // CIC This is for calculate the cycle block by MSHR full (with Pf)
	                           // issue_when - first_issue_try => the cycle block by Pf(occupy MSHR enrty)
                               //
	/* Branch prediction */
