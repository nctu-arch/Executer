...
#include "throttle.h"//cjc_20121215
	...
	unsigned int assoc, enum cache_policy_t policy)
{
	...
	cache->assoc = assoc;
	cache->policy = policy;
	cache->num_of_access_on_set = xcalloc(num_sets, sizeof(long long)); //porshe:20130219 profiling cache accesses distribution of cache lines
...
void cache_free(struct cache_t *cache)
{
	...
	free(cache->num_of_access_on_set);
	free(cache->sets);
	free(cache->name);
	if (cache->prefetcher)
		prefetcher_free(cache->prefetcher);
	if(cache->throttle)
		throttle_free(cache->throttle);//cjc_20121215
	free(cache);
...
void cache_decode_address(struct cache_t *cache, unsigned int addr, int *set_ptr, unsigned int *tag_ptr, 
...
unsigned int cache_set_victim_tag(struct cache_t *cache, int set, int way)
{
	assert(set >= 0 && set < cache->num_sets);
	assert(way >= 0 && way < cache->assoc);
	cache->sets[set].blocks[way].victim_tag= cache->sets[set].blocks[way].tag;
	return cache->sets[set].blocks[way].tag;
}
void cache_set_insert_cycle(struct cache_t *cache, int set, int way,long long cycle_time)
{
	assert(set >= 0 && set < cache->num_sets);
	assert(way >= 0 && way < cache->assoc);
	/*record when the insertion was happened*/
	cache->sets[set].blocks[way].inserted_cycle = cycle_time;
}
long long  cache_get_insert_cycle(struct cache_t *cache, int set, int way)
{
	assert(set >= 0 && set < cache->num_sets);
	assert(way >= 0 && way < cache->assoc);
	assert(cache->sets[set].blocks[way].inserted_cycle);
	return cache->sets[set].blocks[way].inserted_cycle;
}
int cache_get_dead_block_flag(struct cache_t *cache, int set, int way)
{
	assert(set >= 0 && set < cache->num_sets);
	assert(way >= 0 && way < cache->assoc);
	//return cache->sets[set].blocks[way].flag_dead_blck;
	//IC_S_2012_12_27
	return cache->sets[set].blocks[way].dead_block_state;
	//IC_E
}
	...
	//IC_E
{
	assert(set >= 0 && set < cache->num_sets);
	assert(way >= 0 && way < cache->assoc);
	//cache->sets[set].blocks[way].flag_dead_blck = flag;
	//IC_S_2012_12_27
	cache->sets[set].blocks[way].dead_block_state = flag;
	//IC_E
}
void cache_set_invalid_to_tail(struct cache_t *cache, int set, int way)
{
	assert(set >= 0 && set < cache->num_sets);
	assert(way >= 0 && way < cache->assoc);	
	cache_update_waylist(&cache->sets[set],
...
void cache_set_stack(struct cache_t *cache, int set, int way, struct mod_stack_t *stack)
{
	assert(set >= 0 && set < cache->num_sets);
	assert(way >= 0 && way < cache->assoc);
	cache->sets[set].blocks[way].stack=stack;

}
struct mod_stack_t * cache_get_stack(struct cache_t *cache, int set, int way)
{

	assert(set >= 0 && set < cache->num_sets);
	assert(way >= 0 && way < cache->assoc);
	return cache->sets[set].blocks[way].stack;

}

void cache_dump_coherence_state(struct cache_t *cache)
{
	assert(cache);
	int set,way;
	int i,j;
