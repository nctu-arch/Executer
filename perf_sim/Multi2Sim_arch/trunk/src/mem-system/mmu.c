...
//unsigned int mmu_page_size = 1 << 30;  /* 1GB */
//unsigned int mmu_page_size = 0;
...
static struct mmu_page_t *mmu_get_page(int address_space_index, unsigned int vtladdr)
{
	...
	{
		index = 0;
		if(address_space_index)
			fatal("mmu.c:105 Can not support address_space_index > 0");	
	}
...
void mmu_init()
{
	...
	{
		mmu_page_mask = ~0;
		mmu_log_page_size = sizeof(int)*8;
	}
	/* Initialize */
