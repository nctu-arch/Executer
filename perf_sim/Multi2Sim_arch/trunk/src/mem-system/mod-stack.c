	...
	unsigned int addr, int ret_event, struct mod_stack_t *ret_stack)
{
	...
	if (ret_stack != NULL)
	{
		...
		stack->ideal_access = ret_stack->ideal_access;//cjc_20121226 bypass ideal_access flag
		stack->core_id = ret_stack->core_id;//cjc_20130408 bypass core_id
		stack->pc = ret_stack->pc;
	}
	else
	{
	...
	//IC_S_2014_04_08
	stack->pf_hint_priority = xcalloc(1, sizeof(struct pf_hint_t));
	stack->pf_hint_priority->hint_address = 0;
	stack->pf_hint_priority->priority = 2;
	//IC_E
...
void mod_stack_return(struct mod_stack_t *stack)
{
	...
	if(ret_stack) {
		((struct mod_stack_t *)ret_stack)->fetch_level = (stack->fetch_level>((struct mod_stack_t *)ret_stack)->fetch_level)?
		stack->fetch_level:((struct mod_stack_t *)ret_stack)->fetch_level;  //porshe_20121126 0 is L1 hit, 1 is L2 hit, and 2 is memory fetch
		if(stack->peer_to_peer_trans_flag)
			((struct mod_stack_t *)ret_stack)->peer_to_peer_trans_flag = stack->peer_to_peer_trans_flag; //porshe_20121128 init $2$ transfer flag
	}
...
void mod_stack_wakeup_stack(struct mod_stack_t *master_stack)
{
	...
	while (master_stack->waiting_list_head)
	{
		...
		//prefetch = (stack->wait_for_pf)?master_stack->prefetch:prefetch;
	}
	
	if(master_stack->pf_master)
		master_stack->mod->pf_hits++;
	/* Debug */
	mem_debug("\n");
}
