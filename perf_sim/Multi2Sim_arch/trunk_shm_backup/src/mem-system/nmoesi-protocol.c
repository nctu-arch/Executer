	...
	if (event == EV_MOD_NMOESI_FIND_AND_LOCK_PORT)
	{
		...
		if (stack->hit)
			...
			printf("    %lld 0x%x %s hit: set=%d, way=%d, state=%s\n", stack->id,
					stack->tag, mod->name, stack->set, stack->way,
					str_map_value(&cache_block_state_map, stack->state)); //yhchiang
...
#if 1
				printf("%s\n", mod->name);
				if(strcmp(mod->name,"C0L1D")==0)*shm_L1readhit=mod->read_hits;
				else if(strcmp(mod->name,"L2")==0)*shm_L2readhit=mod->read_hits;
#endif
			}
		}
		else if (stack->prefetch)
...
#if 1
				printf("%s\n", mod->name);
				if(strcmp(mod->name,"C0L1D")==0)*shm_L1writehit=mod->write_hits;
				else if(strcmp(mod->name,"L2")==0)*shm_L2writehit=mod->write_hits;
#endif
			}
		}
