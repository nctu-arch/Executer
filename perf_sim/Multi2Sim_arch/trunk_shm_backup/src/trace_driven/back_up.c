...
//			printf("C2 esim_cycle: %lld   %d   %x\n", esim_cycle, kind2, phy_addr_2);
	...
	while(state3 != -1){
		...
		if(time_diff_3 == 0)
		{
//			printf("esim_cycle: %lld   %d   %x\n", esim_cycle, kind3, phy_addr_3);
...
int trace_driven_run_mem()
{
	...
	static char bufferC0[8000]; 
	static char bufferC1[8000]; 
	static char bufferC2[8000]; 
	static char bufferC3[8000];

	basic_trace tmp1;

	if( (issue_c0 == 0 && read_c0 != 0 )|| state0 == 0 )
	{
			state0 =1;
			read_c0 = fread (bufferC0,8,100,mem_access_trace_f_C0);
			issue_c0 = read_c0;
			point0 = bufferC0;
			time_diff_0=0;
			//printf("read_c0: %d \n",read_c0);
	}
	if( (issue_c1 == 0 && read_c1 != 0 )|| state1 == 0 )
	{
			state1 =1;
			read_c1 = fread (bufferC1,8,1000,mem_access_trace_f_C1);
			issue_c1 = read_c1;
			point1 = bufferC1;
			time_diff_1=0;
			//printf("read_c1: %d \n",read_c1);
	}
	if( (issue_c2 == 0 && read_c2 != 0 )|| state2 == 0 )
	{
			state2 =1;
			read_c2 = fread (bufferC2,8,1000,mem_access_trace_f_C2);
			issue_c2 = read_c2;
			point2 = bufferC2;
			time_diff_2=0;
			//printf("read_c2: %d \n",read_c2);
	}
	if( (issue_c3 == 0 && read_c3 != 0 )|| state3 == 0 )
	{
			state3 =1;
			read_c3 = fread(bufferC3,8,1000,mem_access_trace_f_C3);
			issue_c3 = read_c3;
			point3 = bufferC3;
			time_diff_3=0;
			//printf("read_c3: %d \n",read_c3);
...
//	printf("3  %d  %d  \n",issue_c3,read_c3);
	while(read_c0 != 0){
			if(time_diff_0 == 0 && issue_c0 != 0){
					...
					point0 = point0 + 8;
					//if(time_diff_0 != 0 ) break;
					time_diff_0++;
	//		printf("C0 %d  %d  %x \n",kind0,time_diff_0,phy_addr_0);
			}
		...
		if(time_diff_0 == 0 && issue_c0 != 0)
		{
			printf("C0 esim_cycle: %lld   %d   %x\n", esim_cycle, kind0, phy_addr_0);
			if(kind0 == 2)mod_access( (*((*x86_cpu).core[0]).thread).data_mod  , mod_access_load,phy_addr_0, NULL, NULL , NULL , NULL);
			else if(kind0 == 1) mod_access( (*((*x86_cpu).core[0]).thread).data_mod  , mod_access_store,phy_addr_0, NULL, NULL , NULL , NULL);
			issue_c0--;
	...
	//		printf("C1 %d  %d  %x \n",kind1,time_diff_1,phy_addr_1);
		...
		{
		//	printf("C0 esim_cycle: %lld   %d   %x\n", esim_cycle, kind1, phy_addr_1);
			if(kind1 == 2)mod_access( (*((*x86_cpu).core[1]).thread).data_mod  , mod_access_load,phy_addr_1, NULL, NULL , NULL , NULL);
			else if(kind1 == 1) mod_access( (*((*x86_cpu).core[1]).thread).data_mod  , mod_access_store,phy_addr_1, NULL, NULL , NULL , NULL);
			issue_c1--;
			//printf("%d %d %d %d\n",issue_c0,issue_c1,issue_c2,issue_c3);
		}
		else break;
	...
	//		printf("C2 %d  %d  %x \n",kind2,time_diff_2,phy_addr_2);
		...
		if(time_diff_2 == 0 && issue_c2 != 0)
		{
			if(kind2 == 2)mod_access( (*((*x86_cpu).core[2]).thread).data_mod  , mod_access_load,phy_addr_2, NULL, NULL , NULL , NULL);
			else if(kind2 == 1) mod_access( (*((*x86_cpu).core[2]).thread).data_mod  , mod_access_store,phy_addr_2, NULL, NULL , NULL , NULL);
			issue_c2--;
			//printf("%d %d %d %d\n",issue_c0,issue_c1,issue_c2,issue_c3);
		}
		else break;
	...
	while(read_c3 != 0){
			if(time_diff_3 == 0 && issue_c3 != 0){
					...
					point3 = point3 + 8;
	//		printf("C3 %d  %d  %x \n",kind3,time_diff_3,phy_addr_3);
			}
			 if(time_diff_3 != 0)time_diff_3--;

		if(time_diff_3 == 0 && issue_c3 != 0)
		{
			if(kind3 == 2)mod_access( (*((*x86_cpu).core[3]).thread).data_mod  , mod_access_load,phy_addr_3, NULL, NULL , NULL , NULL);
			else if(kind3 == 1) mod_access( (*((*x86_cpu).core[3]).thread).data_mod  , mod_access_store,phy_addr_3, NULL, NULL , NULL , NULL);
			issue_c3--;
			//printf("%d %d %d %d\n",issue_c0,issue_c1,issue_c2,issue_c3);
		}
		else break;
