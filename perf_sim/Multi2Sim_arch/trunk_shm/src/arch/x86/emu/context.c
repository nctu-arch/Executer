...
void x86_ctx_execute(struct x86_ctx_t *ctx)
{
	...
	int spec_mode;
    int next_emu_last_inst_state;  //add by porshe 20130129 forward to parallel
    static int emu_last_inst_state = 0;  //porshe20130129: add state check for serial last instructions
    static int last_match_pc_addr = 0;  //porshe20130129: add state check for serial last instructions
	...
	/*   //marked by porshe_20130129 (orignal)
    ...
    */
    //20130129 add by porshe for forward to parallel
	if (x86_emu_last_inst_size_0 && !emu_last_inst_state &&
		x86_emu_last_inst_size_0 == ctx->inst.size &&
		!memcmp(x86_emu_last_inst_bytes_0, buffer_ptr, x86_emu_last_inst_size_0)){
		assert(last_match_pc_addr == 0);
        last_match_pc_addr = ctx->inst.eip;
        next_emu_last_inst_state = 1;
    }
    else if (x86_emu_last_inst_size_1 && emu_last_inst_state == 1 &&
		x86_emu_last_inst_size_1 == ctx->inst.size &&
		!memcmp(x86_emu_last_inst_bytes_1, buffer_ptr, x86_emu_last_inst_size_1)){
		assert(ctx->inst.eip - last_match_pc_addr == 4);
        last_match_pc_addr = ctx->inst.eip;
        next_emu_last_inst_state = 2;
    }
    else if (x86_emu_last_inst_size_2 && emu_last_inst_state==2 &&
		x86_emu_last_inst_size_2 == ctx->inst.size &&
		!memcmp(x86_emu_last_inst_bytes_2, buffer_ptr, x86_emu_last_inst_size_2)){
		assert(ctx->inst.eip - last_match_pc_addr == 2);
        last_match_pc_addr = ctx->inst.eip;
        next_emu_last_inst_state = 3;
    }
    else{
        next_emu_last_inst_state = 0;
        last_match_pc_addr = 0;
    }
    emu_last_inst_state = next_emu_last_inst_state;
    if(emu_last_inst_state == 3)
        esim_finish = esim_finish_x86_last_inst;
    //end
