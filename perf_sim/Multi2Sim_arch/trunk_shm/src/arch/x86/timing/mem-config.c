...
void x86_mem_config_parse_entry(struct config_t *config, char *section)
{
	...
	X86_THREAD.data_mod->mod_id = core;//cjc_20130207 use core_id as the mod_id
	X86_THREAD.data_mod->data_mod = 1;//cjc_20130207 this is data_module
	...
	X86_THREAD.inst_mod->mod_id = core;//cjc_20130408 use core_id as the mod_id
	
	if(memory_mod!=NULL)
	{
		memory_mod->dram_controller->HPAC_BWC[core] = &(X86_THREAD.data_mod->HPAC_BWC);
		memory_mod->dram_controller->HPAC_BWNO[core] = &(X86_THREAD.data_mod->HPAC_BWNO);
	}
	...
	mem_debug("\t\tEntry for instructions -> %s\tid -> %d\n", X86_THREAD.inst_mod->name,X86_THREAD.inst_mod->mod_id);
	mem_debug("\t\tEntry for data -> %s\tid -> %d\n", X86_THREAD.data_mod->name,X86_THREAD.data_mod->mod_id);
	mem_debug("\n");
}
