...
static void x86_cpu_commit_thread(int core, int thread, int quant)
{
	...
	while (quant && x86_cpu_can_commit_thread(core, thread))
	{
		...
		{	
			...
			//struct mod_t * mod = (((x86_cpu->core[core]).thread[thread]).data_mod);
			if(((X86_CORE.num_committed_inst)%x86_mpki_interval)==0&&esim_warm_up_finish)//cjc_20130124
			{
				if(x86_cpu_mpki_file_name)
				{
					fprintf(MPKI,"core=%d instr=%lld Cycles=%lld Misses=%lld Demand_hit_in_mshr=%lld Pf_from_mem=%lld total_instr=%lld"
							,core,X86_CORE.num_committed_inst,x86_cpu->cycle
							,X86_THREAD.data_mod->accesses-X86_THREAD.data_mod->hits-X86_THREAD.L1Dcache_Misses
							,X86_THREAD.data_mod->pf_late_load-X86_THREAD.pf_mshr
							,X86_THREAD.data_mod->pf_issued_get_from_mem-X86_THREAD.pf_get_from_mem
							,x86_cpu->num_committed_inst
							);
					if(X86_THREAD.data_mod->cache->prefetcher)
					{
						fprintf(MPKI," pf_degree=%d\n",X86_THREAD.data_mod->cache->prefetcher->pf_degree);

					}
					else
						fprintf(MPKI,"\n");
				}					
				//esim_schedule_event(EV_THROTTLE_EVENT_TRIGGER, X86_THREAD.data_mod->cache->throttle,0);
				X86_THREAD.L1Dcache_Misses=X86_THREAD.data_mod->accesses-X86_THREAD.data_mod->hits;
				X86_THREAD.pf_mshr=X86_THREAD.data_mod->pf_late_load;
				X86_THREAD.pf_get_from_mem=X86_THREAD.data_mod->pf_issued_get_from_mem;
				if(X86_THREAD.L1Dcache_Misses<0)
					X86_THREAD.L1Dcache_Misses = 0;

			}
			if(esim_warm_up_finish)
				if(((x86_cpu->num_committed_inst-x86_cpu->num_warm_up_inst)%x86_mem_dump_interval)==0)
				{
					//struct mod_t * L2;
					struct mod_t * MM, *LLC;
					MM = mod_get_main_memory (X86_THREAD.data_mod);
					//L2 = mod_get_low_mod(X86_THREAD.data_mod, 1);
					x86_cpu_dump_per_200M(esim_warm_up_finish);
					//mod_dump_cache_coherence_state(L2);
					LINKED_LIST_FOR_EACH(MM->high_mod_list)
					{
						LLC = linked_list_get(MM->high_mod_list);
						mod_dump_cache_coherence_state(LLC);
					}
				}
		}
		if (uop->flags & X86_UINST_CTRL)
