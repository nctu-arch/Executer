...
static int x86_cpu_issue_sq(int core, int thread, int quant)
{
	...
	while (!linked_list_is_end(sq) && quant)
	{
		...
		{
				// record the fisrt block by mshr full
				// 2013 10 19 cic
		    if(!mod_mshr_can_access(X86_THREAD.data_mod,store->phy_addr) && store->first_issue_try == 0 && (X86_THREAD.data_mod->prefetch_access_list_count != 0) )
					store->first_issue_try = x86_cpu->cycle;
				//CIC
			break;
		}
		...
		//CIC
		if( store->first_issue_try != 0 )
		{
			X86_CORE.block_by_mshr_count++;
			X86_CORE.block_by_mshr_cycle += (store->issue_when - store->first_issue_try);
	    }
		//CIC
...
static int x86_cpu_issue_lq(int core, int thread, int quant)
{
	...
	while (!linked_list_is_end(lq) && quant)
	{
		...
		{
            // CIC add for record mshr full with pf           
		    if(!mod_mshr_can_access(X86_THREAD.data_mod,load->phy_addr) && (load->first_issue_try == 0) && (X86_THREAD.data_mod->prefetch_access_list_count != 0) )
					load->first_issue_try = x86_cpu->cycle;
			//
			linked_list_next(lq);
			continue;
		}
		...
		//CIC
		if( load->first_issue_try != 0 )
		{
			X86_CORE.block_by_mshr_count++;
			X86_CORE.block_by_mshr_cycle += (load->issue_when - load->first_issue_try);
	    }
		//CIC
		//
		/* Statistics */
