...
void dir_entry_unlock(struct dir_t *dir, int x, int y)
{
	...
	if (dir_lock->lock_queue)
	{
		...
		//IC_S_2013_10_29
		f = debug_file(mem_debug_category_2);
		if (f)
		{
			if (dir_lock->lock_queue->dir_lock_next)
			{
...
int dir_entry_is_sharing(struct dir_t *dir, int x, int y, int z)
{
	struct dir_entry_t *dir_entry;

	dir_entry = dir_entry_get(dir, x, y, z);

	return (dir_entry->num_sharers > 1);

}

int dir_entry_get_sharer_count(struct dir_t *dir, int x, int y, int z)
{
	struct dir_entry_t *dir_entry;

        dir_entry = dir_entry_get(dir, x, y, z);

        return dir_entry->num_sharers;

}

int dir_entry_get_the_first_sharer(struct dir_t *dir, int x, int y, int z)
{
        int i;
        for (i = 0; i < dir->num_nodes; i++)
                if (dir_entry_is_sharer(dir, x, y, z, i))
