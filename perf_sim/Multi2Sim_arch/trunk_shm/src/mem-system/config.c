...
//cjc_20121215 throttle prefetcher
#include "throttle.h"

//cjc_20130317 dram 
#include "dram.h"
#include "dram-handler.h" //porshe_20130820

/*
 * Global Variables
 */

int ideal_pf_cache;//cjc_20121219  1225 move to global from mem_config_read_cache 
struct cache_t *pf_cache;//cjc_20121225 pf_cache pointer
...
int HPAC_enable = 0;//cjc_20130924
...
static struct mod_t *mem_config_read_cache(struct config_t *config, char *section)
{
	...
	int prefetcher_stride_degree;	//cjc_20121210
	
	//cjc_20121215
	int throttle_period;		
	//int throttle_evict_count;//
	int throttle_type;
	char *throttle_type_str; 
	int throttle_mothod;
	char *throttle_mothod_str;
	...
	//IC_E
	//cjc_20130202
	double	dead_block_threshold_ratio;
	
	int lower_level_insertion;	
	char *net_name;
	char *net_node_name;
	...
	//cjc_20121217 counters for throttle 
	struct monitor_counter hw_counter;

	//cjc_20130716
	double lateness_ratio;
	double pollution_ratio;
	
	//porshe_20131025: create a simple TLB in L1 module for generating TLB miss info
	int tlb_num_sets;
	int tlb_assoc;
	int tlb_page_size;

	char *tlb_policy_str;
	enum cache_policy_t tlb_policy;
	//end
	
	//IC_S_2013_11_05
	//int L1_hint_history_entry_num;
	//int L1_hint_history_core_num;
	//int L1_hint_history_entry_size;
	//char *L1_hint_history_policy_str;
	//enum cache_policy_t L1_hint_history_policy;
	//IC_E

	//IC_S_2013_12_03
	int L1hint_num_sets;
	int L1hint_assoc;
	int L1hint_block_size;

	char *L1hint_policy_str;
	enum cache_policy_t L1hint_policy;
	//IC_E
	//CIC 20140321	
	int dcu_bidirection;
	...
	prefetcher_type_str = config_read_string(config, buf, "PrefetcherType", "STRIDE");
	...
	prefetcher_stride_degree = config_read_int(config, buf, "Pf_degree", 0);//cjc_20121210
	//pf_on_hit = config_read_int(config, buf, "Pf_on_hit", 1);	
	//throttle_enable = config_read_bool(config, buf, "EnableThrottle", 0);
	throttle_type_str = config_read_string(config, buf, "ThrottleType", "NONE");
	throttle_period = config_read_int(config, buf, "ThrottlePeriod", 50000);
	
	throttle_mothod_str = config_read_string(config, buf,"ThrottleMethod", "PAM");

	pf_distance = config_read_int(config, buf, "Pf_distance", 1);
	if(ideal_pf_cache==0)
		ideal_pf_cache = config_read_int(config, buf, "IdealPfCache", 0);
	if(ideal_pf_cache_contention==0)
		ideal_pf_cache_contention = config_read_int(config, buf, "IdealPfCacheContention", 0);
	if(ideal_pf_cache_pollution==0)
		ideal_pf_cache_pollution = config_read_int(config, buf, "IdealPfCachePollution", 0);
	if(ideal_pf_latency_hiding==0)
		ideal_pf_latency_hiding = config_read_int(config, buf, "IdealPfLatencyHiding", 0);	
	pf_accuracy_ratio=config_read_double(config, buf,"PfAccuracyRatio",0.6);
	pf_accuracy_ratio_low=config_read_double(config, buf,"PfAccuracyRatioLow",0.4);	
	dead_block_threshold_ratio=config_read_double(config, buf, "DeadBlockThresholdRatio", 0.6);	

	pollution_ratio = config_read_double(config, buf, "PollutionThresholdRatio", 0.005);	
	lateness_ratio = config_read_double(config, buf, "LatenessThresholdRatio", 0.01);

	lower_level_insertion = config_read_int(config, buf, "Pf_lower_level_insertion",0);
	fine_grained_ctrl=config_read_int(config, buf, "FineGrainedCtrl",0);
	stride_chain = config_read_int(config, buf, "StrideChain",0);
	dcu_bidirection = config_read_int(config, buf, "DCU_bidirection",0);
	//IC_S_2013_11_05
	L1_DCU_hint = config_read_int(config, buf, "L1DCUHint",0);
	L1_DCU_hint_coreid = config_read_int(config, buf, "L1DCUHintCoreID",0);
	L1_DCU_hint_prefetch_distance = config_read_int(config, buf, "L1DCUHintPrefetchDistance", 1);
	//IC_E
	//IC_S_2014_03_25
	L2_DCU_ActbyHint = config_read_int(config, buf, "L2DCUActbyHint",0);
	//IC_E
	//IC_S_2013_10_15
	stride_coreid_match = config_read_int(config, buf, "Stride_Coreid_Match",0);
	//IC_E
	//IC_S_2013_11_26
	L1_DCU_hint_L2_prefetch_only_be_triggered_by_hint = config_read_int(config, buf, "L1DCUHintL2OnlyHint", 0);
	//IC_E
	//porshe_20131025: create a simple TLB in L1 module for generating TLB miss info
	tlb_num_sets = config_read_int(config, buf, "TLB_Sets", 0);
	tlb_assoc = config_read_int(config, buf, "TLB_Assoc", 1);
	tlb_page_size = config_read_int(config, buf, "PageSize", 4096);
	tlb_policy_str = config_read_string(config, buf, "TLB_Policy", "LRU");
	
	tlb_policy = str_map_string_case(&cache_policy_map, tlb_policy_str);
	if (tlb_num_sets != 0 && tlb_policy == cache_policy_invalid)
		fatal("%s: cache %s: %s: invalid tlb replacement policy.\n%s",
			mem_config_file_name, mod_name,
			tlb_policy_str, err_mem_config_note);
	//end

        //IC_S_2013_12_03
	L1hint_num_sets = config_read_int(config, buf, "L1hint_Sets", 0);
	L1hint_assoc = config_read_int(config, buf, "L1hint_Assoc", 1);
	L1hint_block_size = config_read_int(config, buf, "L1hint_BlockSize", 64); //Total 64 prefetching stream in L2
	L1hint_policy_str = config_read_string(config, buf, "L1hint_Policy", "FIFO");
	L1hint_policy = str_map_string_case(&cache_policy_map, L1hint_policy_str);

	if (L1hint_num_sets != 0 && L1hint_policy == cache_policy_invalid)
		fatal("%s: cache %s: %s: invalid L1hint replacement policy.\n%s",
			mem_config_file_name, mod_name,
			L1hint_policy_str, err_mem_config_note);
		
	//IC_E
	
	//IC_S_2013_11_05
	//L1_hint_history_entry_num = config_read_int(config, buf, "L1_Hint_History_Entry_Num", 0);
	//L1_hint_history_core_num = config_read_int(config, buf, "L1_Hint_History_Core_Num", 1);
	//L1_hint_history_entry_size = config_read_int(config, buf, "L1_Hint_History_Entry_Size", 64);
	//L1_hint_history_policy_str = config_read_string(config, buf, "L1_Hint_Pistory_Policy", "LRU");
	//L1_hint_history_policy = str_map_string_case(&cache_policy_map, L1_hint_history_policy_str);
	//if (L1_hint_history_entry_num != 0 && L1_hint_history_entry_num == cache_policy_invalid)
	//	fatal("%s: cache %s: %s: invalid L1_hint_history replacement policy.\n%s",
	//		mem_config_file_name, mod_name,
	//		L1_hint_history_policy_str, err_mem_config_note);
	//IC_E
	...
	//porshe_20131025: Create tlb if it is L1 cache
	if(tlb_num_sets != 0)
		mod->tlb = cache_create(mod->name, tlb_num_sets, tlb_page_size, tlb_assoc, tlb_policy);
	//end

	//IC_S_2013_12_03
	if(L1hint_num_sets != 0)
	{
		mod->L1hint = cache_create(mod->name, L1hint_num_sets, L1hint_block_size, L1hint_assoc, L1hint_policy);
		mod->L1hint_list0 = list_create_with_size(L1hint_block_size/4); //Default 16 entries/per core
		mod->L1hint_list1 = list_create_with_size(L1hint_block_size/4); //Default 16 entries/per core
		mod->L1hint_list2 = list_create_with_size(L1hint_block_size/4); //Default 16 entries/per core
		mod->L1hint_list3 = list_create_with_size(L1hint_block_size/4); //Default 16 entries/per core
	}
	//IC_E
	
	//IC_S_2013_11_05
	//if(L1_hint_history_entry_num != 0)
	//	mod->L1_hint_history = cache_create(mod->name, L1_hint_history_entry_num, L1_hint_history_entry_size, L1_hint_history_core_num, L1_hint_history_policy);
	//
	//IC_E
	mod->one_fourth_block_mask = ((num_sets*assoc)>>1);
	dead_block_threshold= ((num_sets*assoc)>>2);
	if(mod->one_fourth_block_mask<=0)
		mod->one_fourth_block_mask = 0x10;


	
	/* Fill in prefetcher parameters */
        /*if(enable_prefetcher && prefetcher_stride_degree>=0 && prefetcher_type==prefetcher_type_stride)
        {
                pf_debug("Initialize stride prefetcher owned by \"%s\", ",mod->name);
                //prefetcher_type = str_map_string_case(&prefetcher_type_map, prefetcher_type_str);
                mod->cache->prefetcher = prefetcher_create(prefetcher_ghb_size,pf_distance,prefetcher_stride_degree,prefetcher_type);
        	mod->cache->prefetcher->lower_level_insertion =lower_level_insertion;
		mod->cache->prefetcher->pf_degree = prefetcher_stride_degree;
	}*/
	if (enable_prefetcher)
	{
		pf_debug("Initialize stride prefetcher owned by \"%s\", degree=%d, type=%d\n",mod->name,prefetcher_stride_degree,prefetcher_type);
		mod->cache->prefetcher = prefetcher_create(prefetcher_ghb_size, prefetcher_it_size,
                                                         prefetcher_lookup_depth, prefetcher_type);
		mod->cache->prefetcher->lower_level_insertion =lower_level_insertion;
		mod->cache->prefetcher->pf_degree = prefetcher_stride_degree;
		mod->cache->prefetcher->pf_distance = pf_distance;
		mod->cache->prefetcher->fine_grained_ctrl = fine_grained_ctrl;
		mod->cache->prefetcher->stride_chain = stride_chain;
		mod->cache->prefetcher->dcu_bidirection = dcu_bidirection;
		//IC_S_2013_11_05
		mod->cache->prefetcher->L1_DCU_hint = L1_DCU_hint;
		mod->cache->prefetcher->L1_DCU_hint_coreid = L1_DCU_hint_coreid;
		mod->cache->prefetcher->L1_DCU_hint_prefetch_distance = L1_DCU_hint_prefetch_distance;
		//IC_E
		//IC_S_2014_03_25
		mod->cache->prefetcher->L2_DCU_ActbyHint = L2_DCU_ActbyHint;
		//IC_E
		//IC_S_2013_11_26
		mod->cache->prefetcher->L1_DCU_hint_L2_prefetch_only_be_triggered_by_hint = L1_DCU_hint_L2_prefetch_only_be_triggered_by_hint;
		//IC_E
		//IC_S_2013_10_15
		mod->cache->prefetcher->stride_coreid_match = stride_coreid_match;
		//IC_E
		mod->cache->prefetcher->log_block_size = mod->cache->log_block_size;
		//pf_debug("degree=%d, table_entry=%d\n",prefetcher_stride_degree,prefetcher_ghb_size);
		
	}


        //cjc_20121215 throttle init
	throttle_type = str_map_string_case(&throttle_type_map,throttle_type_str);

	throttle_mothod = str_map_string_case(&throttle_desicion_map,throttle_mothod_str);
	if(throttle_type!=throttle_triggle_type_none&&enable_prefetcher==0)
		fatal("Throttle has no prefetcher\n");
	if(pf_accuracy_ratio_low>pf_accuracy_ratio)
		fatal("Accuracy Threshold Error\n");
	hw_counter.pf_hit=&mod->pf_hits;
	hw_counter.pf_issued=&mod->pf_issued;
	hw_counter.dead_block=&mod->cache_dead_block_demand;
	hw_counter.dead_block_demand_caused_by_pf=&mod->cache_dead_block_demand_caused_by_pf;
	hw_counter.dead_block_threshold=dead_block_threshold;
	hw_counter.pf_accuracy_ratio = pf_accuracy_ratio;
	hw_counter.pf_accuracy_ratio_low = pf_accuracy_ratio_low;
	hw_counter.pf_dead_block_ratio=	dead_block_threshold_ratio;
	hw_counter.FDP_pollution = &mod->pf_pollution_FDP;
	hw_counter.pollution_ratio = pollution_ratio;
	hw_counter.lateness_ratio = lateness_ratio;
	hw_counter.pf_late = &mod->pf_late;
	hw_counter.pf_pollution_HPAC = &mod->pf_pollution_HPAC;
	hw_counter.HPAC_BWNO = &mod->HPAC_BWNO;
	hw_counter.HPAC_BWC = &mod->HPAC_BWC;
		
	mod->cache->throttle=throttle_create(mod->name,mod->cache->prefetcher,throttle_type,throttle_period,&hw_counter);	
	mod->cache->throttle->decision_method = throttle_mothod;
	if(throttle_mothod == throttle_desicion_hpac)
		HPAC_enable = 1;
	//cjc_20121215 throttle 

	prefetcher_pollution_bloom_filter_reset(mod->cache);
	/* Return */
	return mod;
}
...
static struct mod_t *mem_config_read_main_memory(struct config_t *config, char *section)
{
	...
	int dram_ctrl_enable;//cjc_20130317
	int dram_banks;//cjc_20130317
	int dram_channels;//cjc_20130319
	...
	dram_ctrl_enable=config_read_bool(config, section, "EnableDram", 0);
	dram_banks = config_read_int(config, section, "DramBanks", 1);
	dram_channels = config_read_int (config, section, "DramChannels", 1);
	...
	if(((dram_banks*dram_channels)&((dram_banks*dram_channels)-1))!=0)//cjc_20130317
		fatal("%s: %s: dram_banks does not equal to the power of 2.\n%s",
                        mem_config_file_name, mod_name, err_mem_config_note);
	...
	if(dram_ctrl_enable)
	{
		mod->dram_num_banks = dram_banks;
		mod->dram_num_channel =dram_channels;	
		mod->dram_controller = dram_init(mod);
		mod->dram_controller->HPAC_enable = HPAC_enable;
		memory_mod = mod;
		
	}

        dram_init_dramsim2(mod); //porshe_20130820: dram_handler init, register for NUMA design
...
static void mem_config_read_modules(struct config_t *config)
{
	...
	ideal_pf_cache = 0;
	ideal_pf_cache_contention = 0;
	ideal_pf_cache_pollution = 0;
	ideal_pf_latency_hiding = 0;
	pf_cache = NULL;	
	/* Create modules */
...
static void mem_config_calculate_mod_levels(void)
{
	...
	if(ideal_pf_cache)	
	{	
		char name[9]="pf_cache";
		if(ideal_pf_cache_pollution==0)
			pf_cache = cache_create(name, 16, 64, 4096,cache_policy_lru );
		else
			pf_cache = NULL;
	}
	/* Debug */
	...
	for (i = 0; i < list_count(mem_system->mod_list); i++)
	{
		//cjc_20121225 assign pf_cache
		mod = list_get(mem_system->mod_list, i);
		mod->pf_cache = pf_cache ;
		mod->ideal_pf_cache = ideal_pf_cache;
		mod->ideal_pf_cache_contention = ideal_pf_cache_contention;
		mod->ideal_pf_cache_pollution = ideal_pf_cache_pollution;
		mod->ideal_pf_latency_hiding = ideal_pf_latency_hiding;
