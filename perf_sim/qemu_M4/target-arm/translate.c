...
#ifdef CONFIG_TRACE_TIME  //wswu
	cpu_sim_time = tcg_global_mem_new_i32(TCG_AREG0,
        offsetof(CPUState, sim_time), "sim_time");
#endif
...
//sue7785 2013.1.9 Add for timestamp lookup-table
...
gen_traceTicks( int  count )
{
	...
	tcg_gen_addi_i32(cpu_sim_time, cpu_sim_time, count);//wswu
	//tcg_gen_mov_i32(cpu_sim_time, tmp);
...
static void disas_arm_insn(CPUState * env, DisasContext *s)
{
    ...
    insn = ldl_code(s->pc);
    if(start_tracing==1)fprintf(insn_trace_file,"%x\n",insn);
 //sue7785 2013.1.8 Add for timestamp lookup-table
#ifdef CONFIG_TRACE_TIME
        if(start_tracing) {
	...
	//sue7785
	if((insn == 0xe1a0c00c) || (insn == 0xe1a0e00e) ||
	   (insn == 0xe1a0a00a) || (insn == 0xe1a0b00b) ) {
#ifdef CONFIG_TRACE
		gen_helper_traceSwitch(cpu_env);//wswu
#endif
		trace_pid = env->cp15.c13_contpid;
	}
	//sue7785
    ...
    if (cond == 0xf){
			...
			TCGv_i32  count = tcg_temp_new_i32();
        		tcg_gen_movi_i32(count, ticks);
			gen_helper_traceTicks(cpu_env,count);//wswu
 ...
 //sue7785 2013.1.8 Add for timestamp lookup-table
#ifdef CONFIG_TRACE_TIME
        if(start_tracing && ticks > 0 ) {//wswu
                //gen_traceTicks(ticks);
		TCGv_i32  count = tcg_temp_new_i32();
                tcg_gen_movi_i32(count, ticks);
		gen_helper_traceTicks(cpu_env,count);
		tcg_temp_free_i32(count);
        }
#endif
...
//#ifdef TRANSFORMER_KIND
//	if(flag4M_on)seg_cycle_cnt+=read_table(insn);
//#endif
    }
}
...
/* Translate a 32-bit thumb instruction.  Returns nonzero if the instruction
   is not legal.  */
static int disas_thumb2_insn(CPUState *env, DisasContext *s, uint16_t insn_hw1, TranslationBlock* tb/*yhchiang 20160502*/)
{
...
#ifdef TRANSFORMER_KIND
	//printf("%X\n",insn);
	//seg_cycle_cnt+=read_table(insn);
	//tb->cycle += read_table(insn);//yhchiang 20160502
	local_tb_cycle +=read_table(insn);
	//printf("seg_cycle_cnt in translate = %d\n",seg_cycle_cnt);
#endif
        if ((insn & (1 << 12)) == 0) {
            ARCH(5);
            /* Second half of blx.  */
    ...
    //fprintf(insn_trace_file,"KKKKKKKK\n");
    //fprintf(insn_trace_file,"%x\n",insn);
	//sue7785 2013.1.8 Add for timestamp lookup-table
#ifdef CONFIG_TRACE_TIME
        if(start_tracing) {
...
static void disas_thumb_insn(CPUState *env, DisasContext *s, TranslationBlock* tb/*yhchiang 20160502*/)
{
...
#endif
    //printf("%x\n",insn);
    //fprintf(insn_trace_file,"KKKKKKKKK\n");
    //fprintf(insn_trace_file,"%x\n",insn);
	if((insn == 0x46f6) ) {
#ifdef CONFIG_TRACE
		printf("%u\n",env->uncached_cpsr);
  ...
  //sue7785 2013.1.8 Add for timestamp lookup-table
    ...
    case 14:
        if (insn & (1 << 11)) {
            if (disas_thumb2_insn(env, s, insn, tb))//yhchiang
              goto undef32;
            break;
        }
        /* unconditional branch */
    ...
    case 15:
        if (disas_thumb2_insn(env, s, insn, tb))//yhchiang 0502
            goto undef32;
        break;
    }
    return;
                                                  ...
                                                  int search_pc)
{
    ...
    do {
        ...
        if (dc->thumb) {
            disas_thumb_insn(env, dc,tb);//yhchiang
            if (dc->condexec_mask) {
...
done_generating:
    ...
    *gen_opc_ptr = INDEX_op_end;
	...
	tb->cycle = local_tb_cycle;
	//printf("tb->cycle in translate = %d\n",tb->cycle);
	local_tb_cycle = 0;
        tb->icount = num_insns;
    }
#ifdef TRANSFORMER_KIND
    //if(flag4M_on)seg_cycle_cnt += num_insns;//printf("num_insns = %d\n", num_insns);
#endif
}
