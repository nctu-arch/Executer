...
static void SEMA4_mod_writeb(void * opaque, target_phys_addr_t offset, uint32_t value)
{
	...
	printf("current cycle %u  A5 %u\n",tb_total_cycle_cnt, get_A5_cycle());
	printf("value = %d\n",(uint8_t)value);
	if((uint8_t)value==1/*SEM_LOCK*/ ){
		printf("lock@@@@@@@@@@@@@\n");
	while(tb_total_cycle_cnt>get_A5_cycle() && get_A5_cycle()!=0){
		if(first == 0)printf("wait A5 cycle A5 = %d\n",get_A5_cycle());
		first = 1;
		set_global_cycle(tb_total_cycle_cnt);
		if(tb_total_cycle_cnt<=get_A5_cycle())break;
		/*else if(*shm_M0_int == 3){
			printf("go because A5 is asleep\n");
			*shm_M0_int = 1;
			break;
		}*/
	}
