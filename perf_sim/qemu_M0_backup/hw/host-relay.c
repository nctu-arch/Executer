...
static int host_relay_init(SysBusDevice *dev)
{
    ...
    /*shm_int_key = 311;
    shm_int_id  = shmget(shm_int_key,sizeof(int),IPC_CREAT|0666);
    if((shm_int = (int*)shmat(shm_int_id,NULL,0))==-1){
        perror("shmat");
        shmctl(shm_int_id,IPC_RMID,0);
        exit(1);
    }*/
    shm_M0_int_key = 311;
    shm_M0_int_id  = shmget(shm_M0_int_key,sizeof(int),IPC_CREAT|0666);
    if((shm_M0_int = (int*)shmat(shm_M0_int_id,NULL,0))==-1){
        perror("shmat");
        shmctl(shm_M0_int_id,IPC_RMID,0);
        exit(1);
    }
    shm_M4_int_key = 312;
