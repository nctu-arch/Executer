...
//========================yhchiang==============================================================
#ifdef CONFIG_TRACE
		if(env->regs[0]==0x18){ //SYS_EXIT
			//fprintf(stderr,"uuuuuuujjjj@@@@\n");
			//printf("helllllo\n");
			//fflush(stdout);
			//gen_helper_traceSwitch(env);
			if(start_tracing){
				CPUState *cur_cpu;
				tb_flush(env);
				for(cur_cpu=first_cpu;cur_cpu;cur_cpu=cur_cpu->next_cpu){
				        ...
				        fprintf(stderr,"instruction count :  %lu, sim cycle: %lu\n",cur_cpu->inst_count,cur_cpu->sim_time);
				        fprintf(stderr,"load count : %lu , store count : %lu\n",cur_cpu->ld_count,cur_cpu->st_count);
				        //tb_flush(cur_cpu);
