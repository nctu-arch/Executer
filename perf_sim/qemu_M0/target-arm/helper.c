...
//extern uint64_t sim_time;//wswu
extern FILE *trace_log_file;
extern start_tracing;//wswu
extern int      *shm_ptr;
#endif

int now_pid=0; //sue7785
...
//sue7785 2013.1.9 Add for timestamp lookup-table (helper function)
//wswu
#ifdef CONFIG_TRACE
void HELPER(traceSwitch)(CPUState *env)
{
	if(start_tracing){
		printf("in traceSwitch\n");
			CPUState *cur_cpu;
			tb_flush(env);
			for(cur_cpu=first_cpu;cur_cpu;cur_cpu=cur_cpu->next_cpu){
				printf("instruction count :  %lu, sim cycle: %lu\n",cur_cpu->inst_count,cur_cpu->sim_time);
				printf("load count : %lu , store count : %lu\n",cur_cpu->ld_count,cur_cpu->st_count);
				//tb_flush(cur_cpu);
				cur_cpu->inst_count=0;
                        	cur_cpu->sim_time=0;
				cur_cpu->ld_count=0;
				cur_cpu->st_count=0;
			}
			...
			//CPUState *cur_cpu;
                        //for(cur_cpu=first_cpu;cur_cpu;cur_cpu=cur_cpu->next_cpu)tb_flush(env);
			tb_flush(env);
#ifdef CONFIG_TRACE
                        my_trace_init();
#endif
...
void  HELPER(traceTicks)(CPUState *env, uint32_t  ticks)
{
	//printf("%d,  %d\n",env->uncached_cpsr, CPSR_M);
	if((env->uncached_cpsr & CPSR_M)==0x10){
        	//printf("plus one\n");
		env->sim_time += ticks;
		env->inst_count++;
	}
//        fprintf(trace_log_file,"Tick = %d\n", sim_time);
//	my_trace_log(4);//time tick for performance model
//	my_trace_log(ticks);
}
#endif
#endif
