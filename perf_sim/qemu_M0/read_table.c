...
void reg_init(){
	imm=-1;Rm=-1;Rd=-1;Rn=-1;Rt=-1;
	Ra=-1; Rt2=-1; Rlist=-1;RdLo = -1; RdHi = -1;
}
...
void table32_2(uint32_t insn, uint16_t insn_hw1){//111 10 xxxxxx
    ...
    //sprintf(cop2,"%b",op2);
    if(/*op1==0x2 && */op2==0x0 && op==0)table32_9(insn, insn_hw1);//111 10 x0xx xxx  op=0

    else if(/*op1==0x2*/op2==0x20 && op==0)table32_10(insn, insn_hw1);//111 10 x1xx xxx op=0
...
//#endif
	...
	insn_cnt+=1;
        if(checkfor32((insn&0xffff0000)>>16)){
            insn_32 = table32((insn&0xffff0000)>>16, insn);
            is32bit = 1;
	    //varVal32(i_code.vartype);
            ...
            is32bit = 0;
            checkinsn(insn);
	    if((cycle_cnt - cycle_last)==0)i_code.affect = 0;
	    //varVal(i_code.vartype);
        }
	oneloop(0);
        /*if(i_code.affect == 1){
			if(checkLast(i_code_t)){
	...
	oneloop(1);
	if(is32bit && i_code.affect)varVal32(i_code.vartype);
	else if(i_code.affect) varVal(i_code.vartype);
